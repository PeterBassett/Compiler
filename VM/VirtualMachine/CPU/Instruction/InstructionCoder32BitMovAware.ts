...
export default class InstructionCoder32Bit implements InstructionCoder
{
    ...
    decodeInstruction (ram : RAM, offset : number) : { instruction: Instruction, length: number }
    {
        ...
        let opcodeMode : number;
        let sourceRegister : number ;
        let destinationRegister : number;
        let memoryAddress : number;

        opcodeMode =          (codedInstruction & OpcodeMode32BitMask) >>> OpcodeModeOffset;
        sourceRegister =      (codedInstruction & SourceRegister32BitMask) >>> SourceRegisterOffset;
        destinationRegister = (codedInstruction & DestinationRegister32BitMask) >>> DestinationRegisterOffset;
        memoryAddress =       (codedInstruction & MemoryAddress32BitMask) >>> MemoryAddressOffset;  
